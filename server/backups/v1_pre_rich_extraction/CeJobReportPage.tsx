...
import { ArrowLeft, Clock, CheckCircle, AlertTriangle, FileText, Download } from 'lucide-react';
        ...
        <div className="p-8 max-w-6xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                <div className="p-4 bg-slate-50 border-b border-slate-200 font-bold text-slate-700 flex justify-between items-center">
                    <h3>Item Details</h3>
                    <button className="text-xs bg-white border border-slate-300 px-3 py-1.5 rounded hover:bg-slate-50 flex items-center gap-2">
                    ...
                    <table className="w-full text-left text-sm">
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {items.map((item, idx) => {
                                ...
                                return (
                                    <>
                                        <tr key={idx} className={`hover:bg-slate-50 cursor-pointer ${isExpanded ? 'bg-slate-50' : ''}`} onClick={() => setExpandedRow(isExpanded ? null : idx)}>
                                            ...
                                            <td className="p-4 max-w-xs truncate text-blue-600">
                                                <a href={item.product_url} target="_blank" rel="noopener noreferrer" className="hover:underline" onClick={e => e.stopPropagation()}>{item.product_url}</a>
                                            </td>
                                            <td className="p-4 text-slate-500">
                                                {(() => {
                                                    try {
                                                        const mainAssets = item.assets_json ? JSON.parse(item.assets_json).length : 0;
                                                        const variantAssets = variants.reduce((acc: number, v: any) => acc + (v.pdf_urls ? v.pdf_urls.length : 0), 0);
                                                        return <span title={`Main: ${mainAssets}, Variants: ${variantAssets}`}>{mainAssets + variantAssets} found</span>;
                                                    } catch (e) { return 0; }
                                                })()}
                                            </td>
                                            ...
                                            <tr className="bg-slate-50">
                                                <td colSpan={5} className="p-4 pl-12">
                                                    <div className="bg-white border border-slate-200 rounded-lg p-3">
                                                        <h4 className="text-xs font-bold text-slate-500 mb-2 uppercase">Extracted Variants & PDFs</h4>
                                                        <table className="w-full text-xs">
                                                            <thead>
                                                                <tr className="border-b border-slate-100 text-slate-400">
                                                                    <th className="text-left pb-2">Code</th>
                                                                    <th className="text-left pb-2">Dimension</th>
                                                                    <th className="text-left pb-2">PDFs</th>
                                                                </tr>
                                                            </thead>
                                                            <tbody className="divide-y divide-slate-100">
                                                                {variants.map((v: any, vIdx: number) => (
                                                                    <tr key={vIdx}>
                                                                        <td className="py-2 font-mono text-slate-700">{v.variant_code || '-'}</td>
                                                                        <td className="py-2 text-slate-600">{v.dimension}</td>
                                                                        <td className="py-2">
                                                                            {v.pdf_urls && v.pdf_urls.length > 0 ? (
                                                                                <div className="flex flex-col gap-1">
                                                                                    {v.pdf_urls.map((url: string, uIdx: number) => (
                                                                                        <a key={uIdx} href={url} target="_blank" className="text-blue-500 hover:underline truncate max-w-xs block">
                                                                                            📄 {url.split('/').pop()}
                                                                                        </a>
                                                                                    ))}
                                                                                </div>
                                                                            ) : <span className="text-slate-300">No PDFs</span>}
                                                                        </td>
                                                                    </tr>
                                                                ))}
                                                            </tbody>
